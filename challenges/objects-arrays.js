...
const unisWithUni = [];
  unisWithUni.push = universities.filter(uni => uni.includes('Uni'));
...
const lowCaseAnimalNames = [];
    zooAnimals.map(lowerCase => {
       lowCaseAnimalNames.push(lowerCase.animal_name.toLowerCase())
})
...
const lowPopulationAnimals = [];
   zooAnimals.filter(lowCount => {
      if (lowCount.population < 5)
        lowPopulationAnimals.push(`The ${lowCount.animal_name} has a population of ${lowCount.population}.`)
  })
